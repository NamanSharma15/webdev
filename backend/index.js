...
const d = async(dfq)=>{
    console.log(dfq)
    let d = await searchJson2.filter((item)=>
    item.Qualification==dfq.qualification&& 
    item.Gender==dfq.gender &&
    item.Community== dfq.caste &&
    item.ExserviceMen==dfq.exmen &&
    item.Disability==dfq.disable &&
    item.Sports == dfq.sports && 
    item.AnnualPercentage ==dfq.percentage&&
    item.Income ==dfq.income &&
    item.India == dfq.out &&
    item.Religion=="Hindu" &&
    item.Outcome=="1")
    return d;
